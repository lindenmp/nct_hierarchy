...
# %%
n_maps = len(loaders_dict)
n_subs = environment.df.shape[0]
f, ax = plt.subplots(1, n_maps, figsize=(n_maps*5, 5))

for i, key in enumerate(loaders_dict):
    grad_corr = np.zeros(n_subs)

    for j in np.arange(n_subs):
        grad_corr[j] = sp.stats.spearmanr(compute_gradients.gradients[:, 0], loaders_dict[key].values[j, :])[0]

    sns.histplot(x=grad_corr, ax=ax[i])
    ax[i].set_title('corr(grad,{0})'.format(key))
