...
file_label = 'disc_mean_A_s6_{0}_T-{1}_B-{2}-g{3}_E.npy'.format(control, T, B_ver, n_clusters)
print(file_label)
...
E_Am.regress_nuisance(c=mean_over_clusters(A.hops, kmeans.labels_), indices=indices);
...
tm_var_surr = np.load(
    os.path.join(environment.pipelinedir, 'disc_mean_A_s6_{0}_grad{1}_tm_var_surr.npy'.format(surr_type, n_clusters)))
smv_var_surr = np.load(
    os.path.join(environment.pipelinedir, 'disc_mean_A_s6_{0}_grad{1}_smv_var_surr.npy'.format(surr_type, n_clusters)))
joint_var_surr = np.load(os.path.join(environment.pipelinedir,
                                      'disc_mean_A_s6_{0}_grad{1}_joint_var_surr.npy'.format(surr_type, n_clusters)))
...
my_nullplot(mean_over_clusters(A.tm_var, kmeans.labels_)[indices], tm_var_surr[indices], E_Am.data[indices], 'null',
            ax=ax[0])
my_nullplot(mean_over_clusters(A.smv_var, kmeans.labels_)[indices], smv_var_surr[indices], E_Am.data[indices], 'null',
            ax=ax[1])
