...
sns.violinplot(data=df_plot, ax=ax, inner="box", palette="pastel", cut=2, linewidth=1.5)
...
ax.text(0.5, ax.get_ylim()[1]+ax.get_ylim()[1]*0.15, textstr,
        horizontalalignment='center', verticalalignment='top')
ax.axhline(y=ax.get_ylim()[1]-ax.get_ylim()[1]*0.05, xmin=0.25, xmax=0.75, color='k', linewidth=1)
f.savefig(os.path.join(environment.figdir, 'e_{0}.svg'.format(B)), dpi=300, bbox_inches='tight',
          pad_inches=0.1)
plt.close()
...
ax.set_xticklabels('')
ax.tick_params(pad=-2.5)
f.savefig(os.path.join(environment.figdir, 'e_asym_{0}.svg'.format(B)), dpi=300, bbox_inches='tight', pad_inches=0.1)
plt.close()
...
f.savefig(os.path.join(environment.figdir, 'e_asym_hyperplane_{0}.svg'.format(B)), dpi=300,
...
f.savefig(os.path.join(environment.figdir, 'e_asym_hyperplane_network_null_{0}.svg'.format(B)), dpi=300,
...
f.savefig(os.path.join(environment.figdir, 'corr(e_asym_{0},ec_asym).svg'.format(B)), dpi=300, bbox_inches='tight',
          pad_inches=0.1)
plt.close()
...
if B != 'wb':
    ...
    f.savefig(os.path.join(environment.figdir, '{0}_lat.svg'.format(B)), dpi=300, bbox_inches='tight',
              pad_inches=0)
    plt.close()
    ...
    f.set_figwidth(figwidth)
    f.set_figheight(figheight)
    f.savefig(os.path.join(environment.figdir, '{0}_med.svg'.format(B)), dpi=300, bbox_inches='tight',
              pad_inches=0)
    plt.close()
    ...
    f.savefig(os.path.join(environment.figdir, 'corr({0},energy_{0}).svg'.format(B)), dpi=300, bbox_inches='tight',
              pad_inches=0.1)
    plt.close()
    ...
    f.savefig(os.path.join(environment.figdir, 'e_asym_mean_spin_null_{0}.svg'.format(B)), dpi=150,
              bbox_inches='tight', pad_inches=0.1)
    plt.close()
    ...
    f.savefig(os.path.join(environment.figdir, 'e_asym_mean_network_null_{0}.svg'.format(B)), dpi=150,
              bbox_inches='tight', pad_inches=0.1)
    plt.close()
